...
const std::string path::log_dir() {
    ...
    boost::filesystem::path full_path = 
		boost::filesystem::system_complete(
		    boost::filesystem::path(ret.c_str(), boost::filesystem::native));
...
const std::string path::working_dir_win_32() {
    std::string ret = "c:/Program Files/SmartFiles";
    return ret;
}

const std::string path::smartfiles_path() {
    return
#if defined(__APPLE__)
...
const std::string path::win_32_path() {
    ...
    ;
#endif
    return std::string("\"" + prefix + "\\SmartFiles\\SmartFiles.lnk\"");
}
...
const std::string path::java_application_stub() {
    ...
    log_debug("Path to SmartFiles executable(%s)" << ret);
